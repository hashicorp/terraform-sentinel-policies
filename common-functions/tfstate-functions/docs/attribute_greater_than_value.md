# [attribute_greater_than_value](../tfstate-functions.sentinel#L561)
...
```
highCPUVMs = state.attribute_greater_than_value(allVMs,
             "num_cpus", maxCPUs, true)


highMemoryVMs = state.attribute_greater_than_value(allVMs,
                "memory", maxMemory, true)

violatingDisks = state.attribute_greater_than_value(disks,
